...
deploy:
    contract: ./rholang/examples/hello_world_again.rho
    phlo_limit: 100000
    phlo_price: 1
    ...
    def deploy_and_propose(self, deploy_key, contract, phlo_price, phlo_limit):
        with grpc.insecure_channel(self.grpc_host) as channel:
            ...
            try:
                ...
                logging.error("gRPC error occurred: {}".format(rpc_error))
                return None
...
class DispatchCenter():
    ...
    def wait_next_server_to_receive(self, block_hash):
        ...
        if not self.queue:
            logging.error("All nodes are inaccessible or the queue is empty. Exiting.")
            return False
        current_time = int(time.time())
        wait_server = self.queue.popleft()
        client = self.clients.get(wait_server)
        if not client:
            logging.error("Client for server {} not found. Skipping.".format(wait_server))
            return False
    ...
    def run(self):
        ...
        while self._running and self.queue:
            self.update_queue()
            if not self.queue:
                logging.error("All nodes have failed. Stopping the dispatcher.")
                self._running = False
                break
            ...
            if block_hash is not None:
                ...
                logging.error("Failed to get a block hash. Retrying...")
                time.sleep(self.wait_interval)
